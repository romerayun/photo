...
<article x-data="lightbox(@js($lightboxData))" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <a href="{{ route('portfolio.index', ['locale' => $locale]) }}" 
               class="text-xs uppercase tracking-widest text-neutral-400 hover:text-white inline-flex items-center gap-1.5 transition-colors">
                <span>&larr;</span>
        ...
        <div class="mb-8 p-4 rounded-2xl bg-apple-card border border-white/10 text-xs text-neutral-400 flex items-center justify-between">
            <span>{{ __('site.untranslated_notice') }}</span>
            <a href="{{ route('series.show', ['locale' => 'ru', 'slug' => $series->slug]) }}" class="text-apple-blue underline font-semibold">
                Читать на русском
            </a>
        </div>
        @endif
        ...
        <div class="border-b border-white/10 pb-12 mb-16">
            <div class="max-w-4xl space-y-4">
                
                {{-- HUD Tags --}}
                <div class="flex flex-wrap items-center gap-3 text-xs uppercase tracking-widest text-neutral-400 font-mono">
                    @if($series->category)
                    <span class="text-apple-blue font-bold">{{ $series->category->localizedName($locale) }}</span>
                    <span>&bull;</span>
                    @endif
                    ...
                    @if($series->is_demo)
                    <span class="bg-apple-blue/20 text-apple-blue px-2.5 py-0.5 rounded-full text-[0.65rem] font-bold">
                        Demo
                    </span>
                    @endif
                </div>

                <h1 class="text-4xl sm:text-6xl md:text-7xl font-extrabold tracking-tightest apple-text-gradient">
                    {{ $series->localizedTitle($locale) }}
                </h1>

                @if($series->localizedDescription($locale))
                <p class="text-base sm:text-xl text-neutral-300 leading-relaxed max-w-2xl font-light pt-2">
        ...
        <div class="space-y-12 md:space-y-20">
            @foreach($series->photos as $index => $photo)
                ...
                <figure class="relative group cursor-pointer {{ $isWide ? 'max-w-6xl mx-auto' : 'max-w-4xl mx-auto' }}" 
                    ...
                    <div class="overflow-hidden rounded-3xl bg-neutral-900 border border-white/10 shadow-2xl relative">
                        <img src="{{ $photo->url }}" 
                        ...
                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-end justify-end p-6 opacity-0 group-hover:opacity-100">
                            <span class="apple-glass rounded-full text-white text-xs px-4 py-2 font-mono uppercase tracking-wider">
                                Увеличить &plus;
                            </span>
                        </div>
                    </div>
                    ...
                    <figcaption class="mt-3 text-xs text-neutral-400 font-light italic text-center">
        ...
        <div class="mt-28 pt-16 border-t border-white/10">
            <div class="flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
                ...
                <div class="space-y-2">
                    ...
                    <h3 class="text-2xl sm:text-4xl font-bold text-white tracking-tight">
                        {{ __('site.similar_cta') }}
                    </h3>
                ...
                <div class="flex flex-col sm:flex-row items-center gap-4 shrink-0">
                    <a href="{{ route('contacts.index', ['locale' => $locale]) }}" 
                       class="px-8 py-4 bg-white text-black hover:bg-neutral-200 text-xs uppercase tracking-widest font-bold rounded-full transition-all shadow-[0_0_25px_rgba(255,255,255,0.25)]">
                        {{ __('site.similar_cta') }}
                    </a>

                    @if($nextSeries)
                    <a href="{{ route('series.show', ['locale' => $locale, 'slug' => $nextSeries->slug]) }}" 
                       class="px-7 py-4 bg-white/10 hover:bg-white/20 text-white text-xs uppercase tracking-widest font-semibold rounded-full border border-white/15 transition-all inline-flex items-center gap-2">
    ...
    <div x-show="isOpen" 
        ...
        <button @click="close()" 
                type="button" 
                class="absolute top-6 right-6 text-white/80 hover:text-white p-3 rounded-full apple-glass transition-colors z-50"
        ...
        <button @click="prev()" 
                type="button" 
                class="absolute left-6 top-1/2 -translate-y-1/2 text-white/80 hover:text-white p-3.5 rounded-full apple-glass transition-colors z-50"
        ...
        <button @click="next()" 
                type="button" 
                class="absolute right-6 top-1/2 -translate-y-1/2 text-white/80 hover:text-white p-3.5 rounded-full apple-glass transition-colors z-50"
        ...
        <div class="relative max-w-full max-h-[88vh] flex flex-col items-center justify-center select-none" @click.away="close()">
            ...
            <div class="mt-4 flex items-center justify-between w-full text-xs text-neutral-400 font-mono px-4">
                <span x-text="currentPhoto().caption || currentPhoto().alt || ''"></span>
                <span class="apple-glass px-3 py-1 rounded-full text-white" x-text="(currentIndex + 1) + ' / ' + photos.length"></span>
            </div>
        </div>
