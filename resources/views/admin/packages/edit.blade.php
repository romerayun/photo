...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
            <h1 class="text-2xl font-serif font-bold text-slate-900">Редактирование пакета «{{ $package->title_ru }}»</h1>
    ...
    <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="space-y-6 bg-white p-6 sm:p-8 border border-slate-200 rounded-xl shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="photo_count_ru" id="photo_count_ru" value="{{ old('photo_count_ru', $package->photo_count_ru) }}" placeholder="Например: от 40 кадров в обработке"
                       class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900">
            </div>
            ...
            <div>
                ...
                <input type="text" name="delivery_time_ru" id="delivery_time_ru" value="{{ old('delivery_time_ru', $package->delivery_time_ru) }}" placeholder="Например: до 10 дней"
                       class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900">
            </div>
            ...
            <div class="flex items-center pt-6">
                <label class="flex items-center text-sm font-semibold text-slate-800 cursor-pointer select-none">
                    <input type="checkbox" name="is_price_from" value="1" {{ old('is_price_from', $package->is_price_from) ? 'checked' : '' }} class="w-4 h-4 rounded border-slate-300 text-crimson focus:ring-crimson mr-2.5">
        ...
        <div>
            ...
            <textarea name="includes_ru" id="includes_ru" rows="5"
                      ...
                      class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900 leading-relaxed">{{ old('includes_ru', $package->includes_ru) }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="extra_conditions_ru" id="extra_conditions_ru" rows="2"
                      ...
                      class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900 leading-relaxed">{{ old('extra_conditions_ru', $package->extra_conditions_ru) }}</textarea>
        </div>

        {{-- Published Toggle --}}
        <div class="border-t border-slate-200 pt-6">
            <label class="flex items-center text-sm font-semibold text-slate-800 cursor-pointer select-none">
                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $package->is_published) ? 'checked' : '' }} class="w-4 h-4 rounded border-slate-300 text-crimson focus:ring-crimson mr-2.5">
        ...
        <div class="border-t border-slate-200 pt-6 flex items-center justify-between">
            <div>
                <button type="submit" form="delete-package-form" class="px-4 py-2.5 bg-rose-50 text-rose-700 border border-rose-200 text-xs uppercase tracking-wider font-bold hover:bg-rose-100 transition-colors rounded-lg cursor-pointer">
                    Удалить пакет
                </button>
            </div>
    ...
    <form id="delete-package-form" method="POST" action="{{ route('admin.packages.destroy', $package) }}" class="hidden" onsubmit="return confirm('Вы действительно хотите удалить пакет «{{ $package->title_ru }}»?');">
        @csrf
        @method('DELETE')
    </form>
