...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
            <h1 class="text-2xl font-serif font-bold text-slate-900">Редактирование вопроса FAQ</h1>
            <p class="text-xs text-slate-500 mt-0.5">Изменение текста вопроса, ответа и статуса публикации.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.faqs.update', $faq) }}" class="space-y-6 bg-white p-6 sm:p-8 border border-slate-200 rounded-xl shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="question_ru" id="question_ru" value="{{ old('question_ru', $faq->question_ru) }}" required
        ...
        <div>
            ...
            <textarea name="answer_ru" id="answer_ru" rows="4"
                      ...
                      class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900">{{ old('answer_ru', $faq->answer_ru) }}</textarea>
        </div>

        {{-- Collapsible English Fields --}}
        <div class="pt-2 border-t border-slate-100" x-data="{ openEn: {{ (old('question_en', $faq->question_en) || old('answer_en', $faq->answer_en)) ? 'true' : 'false' }} }">
            ...
            <div x-show="openEn" x-cloak class="mt-4 space-y-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
                ...
                <div>
                    ...
                    <textarea name="answer_en" id="answer_en" rows="3"
                              ...
                              class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-neutral-900 bg-white">{{ old('answer_en', $faq->answer_en) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
            ...
            <div class="flex items-center sm:pt-7">
                <label class="inline-flex items-center gap-3 cursor-pointer select-none">
                    <input type="checkbox" name="is_draft" value="1" {{ old('is_draft', $faq->is_draft) ? 'checked' : '' }}
        ...
        <div class="pt-4 border-t border-slate-200 flex items-center justify-between">
            <button type="button" onclick="if(confirm('Удалить этот вопрос?')) document.getElementById('delete-faq-form').submit();" class="text-xs text-rose-600 hover:text-rose-800 font-semibold cursor-pointer">
                Удалить вопрос
            </button>
    ...
    <form id="delete-faq-form" method="POST" action="{{ route('admin.faqs.destroy', $faq) }}" class="hidden">
        @csrf
        @method('DELETE')
    </form>
